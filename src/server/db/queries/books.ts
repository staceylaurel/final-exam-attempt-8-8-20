...
const insert = (newBook: {title: string, author: string, price: string, categoryid: number}) => Query('UPDATE books SET ? WHERE id = ?', [newBook]);

const update = (editedBook: {title?: string, author?: string, price?: string, categoryid?: number}, id: number) => Query('UPDATE INTO books SET ?', [editedBook, id]);
